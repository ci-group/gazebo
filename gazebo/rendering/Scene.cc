...
bool Scene::ProcessLinkMsg(ConstLinkPtr &_msg)
{
  ...
  if (!this->GetVisual(linkVis->GetName() + "_COM_VISUAL__"))
  {
    this->CreateCOMVisual(_msg, linkVis);
  }

  if (!this->GetVisual(linkVis->GetName() + "_INERTIA_VISUAL__"))
  {
    this->CreateInertiaVisual(_msg, linkVis);
  }

  if (!this->GetVisual(linkVis->GetName() + "_LINK_FRAME_VISUAL__"))
  {
